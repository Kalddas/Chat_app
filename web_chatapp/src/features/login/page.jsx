...
function LoginForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.account_suspended || result.status === "Suspended" || result.user?.profile?.status === "Suspended") {
        setSuspendedToken(result.token);
      ...
      if (result.needs_password_change) {
        ...
        const from = location.state?.from?.pathname || (result.user.role === "admin" ? "/admin" : "/chat");
        navigate(from, { replace: true });
      }
    } catch (err) {
      ...
      // Normalize common invalid-credential responses
      if (
        status === 401 ||
        msg === "auth.failed" ||
        (typeof msg === "string" && msg.toLowerCase().includes("credential"))
      ) {
        const m = t("errors.invalidCredentials", { defaultValue: "Invalid email or password" });
        setError(m);
        toast.error(m);
      } else if (msg === "Email not Verified" || msg === "Email not verified") {
        const m = t("errors.emailNotVerified", { defaultValue: "Please verify your email before logging in" });
        setError(m);
        toast.error(m);
      } else if (msg && msg.includes("banned")) {
        const m = t("errors.accountBanned", { defaultValue: "Your account has been banned and cannot be accessed" });
        setError(m);
        toast.error(m);
      } else if (msg === "Account is not active") {
        const m = t("errors.accountNotActive", { defaultValue: "Your account is not active. Please contact admin." });
        setError(m);
        toast.error(m);
      } else {
        const m = msg || t("errors.generic");
        setError(m);
        toast.error(m);
      }
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {error && (
                <Alert variant="destructive" className="rounded-lg">
                  <AlertDescription>{error || t("errors.generic")}</AlertDescription>
                </Alert>
              )}
